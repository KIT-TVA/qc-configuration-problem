...
def init_sat_circuit(problem: List[List[Tuple[int, bool]]]) -> (int, int, QASM3, QASM3):
    ...
    oracle.add_qubit("in_reg", num_vars)
    qasm.add_qubit("in_reg", num_vars)
    ...
    qasm.add_bit("c", num_vars)

    create_ksat_oracle(oracle, "in_reg", ("tar", 0), problem)
    oracle_converter(oracle, ("tar", 0))

    add_all_hadamards(qasm, "in_reg", list(range(num_vars)))

    return num_vars, num_clauses, oracle, qasm

def add_diffuser(qasm: QASM3, inp_reg: str, num_qubits: int):
    add_all_hadamards(qasm, "in_reg", list(range(num_qubits)))
    create_not_oracle(qasm, "in_reg", list(range(num_qubits)))
...
def create_ksat_grover(problem: List[List[Tuple[int, bool]]], k: int) -> QASM3:
    ...
    for i in range(k):
        ...
        add_diffuser(qasm, "in_reg", num_inp_qubits)

    return qasm

def create_grover_for_model(rel_path: str, k:int = 1) -> QASM3:
    # load given model
    ...
    return create_ksat_grover(problem, k)
